...
public class StudentRegistration {
	...
	public double getFinalGrade() {
		double finalGrade = (course.getFinalExamsGradeWeight() * this.finalExamsGrade) + (course.getProjectGradeWeight() * this.projectGrade);
		int decimal = (int) finalGrade; 
		double fractional = finalGrade - decimal; 
		
		if (fractional > 0.75)
			return Math.ceil(finalGrade);
		else if (fractional > 0.5 || fractional > 0.25)
			return decimal + 0.5;
	...
	public String toString() {
		return "StudentRegistration [id=" + id + ", name=" + name + ", yearOfRegistration=" + yearOfRegistration
				...
				+ ", finalGrade=" + finalGrade + ", course=" + course + "]";
	}
